...
folders_LOVSoc = cellfun(@(folders_with_overview) ~isempty(folders_with_overview), strfind(folders_with_overview, 'LOVSoc'));
...
for j = find(~folders_LOVSoc)
    plot (all_exp_num{1,j}, ROI_control_all{1,j},'marker', '.', 'color',  [0.5 0.5 0.5])
    hold on 
    %plot (all_exp_num{1,j}, ROI_ill_all{1,j}, 'marker', '.', 'color', rand(1,3))
